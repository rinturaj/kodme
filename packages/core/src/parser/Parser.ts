...
export class Parser {
  ...
  private declaration(): Stmt | null {
    try {
      ...
      if (
        ...
        this.checkNext(TokenType.Assignment)
      ) {
  ...
  private ifStatement(): Stmt {
    ...
    this.consume(
      ...
      "Expect indentation (block) after if statement.",
    );
  ...
  private repeatStatement(): Stmt {
    ...
    if (this.match(TokenType.Until)) {
      ...
      this.consume(
        TokenType.Newline,
        "Expect newline after repeat until condition.",
      );
  ...
  private unary(): Expr {
    if (this.match(TokenType.NotEqual, TokenType.Minus)) {
      // ! or -
      const operator = this.previous();
      const right = this.unary();
  ...
  private primary(): Expr {
    ...
    if (this.match(TokenType.Ask)) {
      const promptToken = this.consume(
        TokenType.String,
        "Expect prompt string after 'ask'.",
      );
      return new AskExpr(promptToken.value);
    }

    if (this.match(TokenType.Unknown)) {
      // LeftParen?
