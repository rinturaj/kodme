  ...
  private handleIndentation(currentIndent: number): void {
    ...
    if (currentIndent > previousIndent) {
      ...
      while (currentIndent < this.indentStack[this.indentStack.length - 1]) {
        this.indentStack.pop();
        this.tokens.push(
          Token.from("", TokenType.Dedent, 0, 0, this.lineIndex + 1),
        );
      }
      if (currentIndent !== this.indentStack[this.indentStack.length - 1]) {
        throw new Error(
          `Inconsistent indentation at line ${this.lineIndex + 1}`,
        );
      }
    }
  }
    ...
    while (currentLine.length > 0) {
      ...
      if (!matched) {
        ...
        if (currentLine[0] === " " || currentLine[0] === "\t") {
          ...
          throw new Error(
            `Unexpected token at line ${this.lineIndex + 1}: '${currentLine[0]}'`,
          );
        }
      }
    }
